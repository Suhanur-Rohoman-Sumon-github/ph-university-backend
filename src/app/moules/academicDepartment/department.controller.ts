...
const createDepartmemt = catchAsync(async (req, res) => {
  const result =
    await DepartmentServices.createDepartmentIntoDB(req.body);
...
const getAllDepartments = catchAsync(async (req, res) => {
  const result =
    await DepartmentServices.getAllDepartmentsFromDB();
...
const getSingleDepartment = catchAsync(async (req, res) => {
  ...
  const result =
    await DepartmentServices.getSingleDepartmentFromDB(
      departmentId,
    );
...
const updateDeartment = catchAsync(async (req, res) => {
  const { departmentId } = req.params;
  const result =
    await DepartmentServices.updateDepartmentIntoDB(
      departmentId,
      req.body,
    );
